...
Сервис принимает условия события и возвращает до трёх подрядчиков с конкретными объяснениями выбора. Ниже разделы 1–6 описывают воспроизводимый подбор исходного демокаталога по `/demo`.

Живой каталог `/` использует `WorkspaceCatalogRepository`: только проверенные опубликованные профили Firestore. `LiveRecommendationService` читает нужный календарный месяц и передаёт движку доступность; неполученное или устаревшее (30 дней) подтверждение исключает профиль. Для live действует горизонт 365 дней, а данные заказчика сохраняются в аккаунте. Кабинеты работают на Spark; [подключение](firebase.md), [архитектура](architecture.md).
...
Этот раздел сохраняет общий принцип серверной проверки фактов. Реализованный отдельный помощник `/assistant`, его callable-контракт и требования к настоящему ключу/эмбеддингам описаны в [assistant.md](assistant.md). Он работает только с демонстрационным набором. Живые кабинеты и их сохранённые подборки не требуют AI-сервера.
