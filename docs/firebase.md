...
- Живой каталог не подменяется демоданными. Исходные 66 анкет доступны отдельно по `/demo`.
...
В эмуляторном режиме Firebase инициализируется отдельными тестовыми идентификаторами и проектом `demo-event-match`, без конфигурации рабочего проекта. Можно задать `--dart-define=FIREBASE_EMULATOR_PROJECT_ID=demo-custom`; такое же имя передайте Firebase CLI. Допускается только префикс `demo-`. Auth и Firestore направляются на эмуляторы.
