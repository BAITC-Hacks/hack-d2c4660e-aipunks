...
- Живой каталог не подменяется демоданными. Исходные 66 анкет доступны отдельно по `/demo`; демопомощник доступен по `/demo/assistant`. Основной AI-подбор встроен в живой каталог; `/assistant` перенаправляет в него.
