...
Основной подбор встроен в каталог `/`: свободный запрос, редактируемые условия, уточнения и карточки находятся на одной странице. Из мероприятия или плана передаётся контекст; результат можно сохранить в это мероприятие, добавить подрядчика в избранное или перейти к обращению. `/assistant` перенаправляет в каталог. Отдельный учебный разговор с 66 исходными анкетами доступен по `/demo/assistant`; живые профили и демоданные не смешиваются. Бронирование не выполняется.

Решение и первичные UX-источники описаны в [исследовании](research/integrated-assistant-ux.md); найденные дефекты и границы проверки — в [аудите](audit-integrated-assistant.md).
...
Живой каталог использует горизонт от сегодняшнего дня Казахстана до +365 дней и подтверждение месяца не старше 30 дней. Занятость проверяется заново после изменения даты. Календарь демокаталога покрывает 23.09–31.12.2026. Дата вне окна сохраняется как пожелание, но доступность на неё не подтверждается. Бюджет относится к одному подрядчику; общий бюджет события не применяется как лимит для специалиста.
...
- Flutter: встроенный подбор, контекст мероприятия, редактируемые условия, календарь/бюджет, предварительные карточки, сравнение, причины отказа, избранное/обращение, повтор, защита от устаревшего ответа и явный ручной режим. Черновик плана переживает переходы между страницами в текущей сессии.
...
- Embeddings не фабрикуются. В live-режиме векторы подходящих опубликованных описаний рассчитываются пакетами и кэшируются по содержимому и ревизии; запросы кэшируются отдельно по UID. Динамические описания не получают вымышленных проверенных признаков: неподтверждённые пожелания остаются явно неизвестными. Для демокаталога реальные векторы можно подготовить отдельной командой после настройки ключа.
...
Откройте каталог: подбор находится сразу на странице. Для проверки исходных анкет откройте `/demo/assistant`. Для Android Emulator используется `10.0.2.2`; для браузера `127.0.0.1`. На физическом телефоне задайте `--dart-define=FIREBASE_EMULATOR_HOST=<адрес_компьютера>` и настройте сетевой доступ эмуляторов отдельно.
...
Для `source: live` сервер заново читает опубликованные профили, проверяет активность аккаунта и календарь. Поля профиля выбираются явно; приватные данные не передаются модели. Файлы `functions/data/catalog.json` и `assistant_catalog.json` воспроизводимо собираются скриптом. Источник `apps/event_match/assets/data/catalog.jsonl` не меняется. Для новых профилей требуется проверенная разметка; скрипт намеренно не принимает незаметное изменение количества или состава исходных 66.
...
В эмуляторе callable использует memory-кэш по умолчанию. Для проверки полного пути через Firestore задайте `ASSISTANT_STORAGE=firestore`. В облаке используется только Firestore. TTL по `expiresAt` для `assistant_cache` и `assistant_rate_limits` объявлен в `firestore.indexes.json`; политики требуют применения в облаке и биллинга. Срок годности проверяется при чтении независимо от фонового удаления. Приватные записи содержат `ownerUid`; CLI удаления аккаунта очищает их. Tombstone-проверка в транзакции блокирует позднюю повторную запись после удаления.
...
Live smoke: `functions/scripts/smoke_live.mjs` проверяет callable с настоящими Firestore/Auth Emulator: источник, календари, снятие публикации, блокировку и удаление аккаунтов. Он не проверяет качество OpenAI. При запуске через `firebase.assistant.json` задайте `ASSISTANT_STORAGE=firestore`.
