...
- `/` содержит встроенный live-подбор с `AssistantSession`; `/assistant` перенаправляет сюда. `/demo/assistant` сохраняет отдельный деморежим. [Серверная настройка](assistant.md) нужна для свободного текста; типизированный подбор работает без ключа.
