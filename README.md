...
- [Умный подбор внутри каталога](docs/assistant.md) · [контракт](docs/assistant-contract.md)
...
| Умный подбор в `/` | Запрос одной фразой, контекст мероприятия, редактирование условий, live-рекомендации и прямые действия |
...
Подтверждение календарного месяца действует 30 дней. Отсутствующий или устаревший месяц не означает свободную дату: профиль остаётся в каталоге, но не входит в подбор. Цена «от» и календарь не являются бронированием. Оплат и загрузки файлов нет; портфолио — HTTPS-ссылки.
...
apps/event_match/lib/features/assistant/  встроенный live-подбор и деморежим
