...
import { evaluate, hasSearchMinimum, recommend } from "./engine.mjs";
import { ALGORITHM_VERSION } from "./types.mjs";
function choices(catalog, brief, field) {
    ...
    const source = field === "category" ? (pool.length ? pool : catalog.contractors).flatMap((item)=>item.categories) : field === "city" ? (pool.length ? pool : catalog.contractors).map((item)=>item.city) : field === "event_format" ? pool.flatMap((item)=>item.event_formats) : field === "language" ? pool.flatMap((item)=>item.languages) : [];
    return [
        ...new Set(source)
    ].sort((a, b)=>a.localeCompare(b, "ru"));
}
function fieldQuestion(catalog, brief, field) {
    const prompts = {
        category: "Какого подрядчика ищете?",
        city: "В каком городе пройдёт мероприятие?",
        event_format: "Для какого мероприятия подбираем подрядчика?",
        date: "На какую дату проверить занятость?",
        ...
        hours: "На сколько часов нужен подрядчик?",
        language: "На каком языке должно пройти мероприятие?"
    };
    ...
    if (field === "budget_kzt") {
        const prices = [
            ...new Set(evaluate(catalog, {
                ...brief,
                budget_kzt: null,
                budget_scope: "contractor"
            }).eligible.map((item)=>item.contractor.price_from_kzt))
        ].sort((a, b)=>a - b);
        const thresholds = prices.length ? [
            ...new Set([
                prices[0],
...
function informativeFeature(catalog, brief) {
    ...
    return catalog.featureDefinitions.map((definition)=>{
        const support = pool.filter((item)=>(catalog.features.get(item.contractor.id) ?? []).some((feature)=>feature.key === definition.key && feature.polarity === "supports")).length;
        return {
            key: definition.key,
            split: support && support < pool.length ? Math.min(support, pool.length - support) : 0
        };
    }).sort((a, b)=>b.split - a.split || a.key.localeCompare(b.key, "en")).find((item)=>item.split > 0)?.key;
}
function refinementActions(brief) {
    return [
        actionFor(brief, "Изменить бюджет", "pick_budget", "budget_kzt"),
        actionFor(brief, "Изменить дату", "pick_date", "date"),
        actionFor(brief, "Найти другого специалиста", "next_category", undefined, null)
    ];
}
export function buildTurn(catalog, brief, options) {
    let result = hasSearchMinimum(brief) ? recommend(catalog, brief, options.semanticScores) : null;
    ...
    const missingCore = [
        ...
        "city",
        "event_format"
    ].find((field)=>brief[field] === null);
    const missing = [
        "date",
        "budget_kzt"
    ].find((field)=>(brief[field] === null || field === "budget_kzt" && brief.budget_scope === "event") && !brief.skipped_fields.includes(field));
    if (missingCore) {
        ...
        if (result.outcome === "category_absent") {
            actions = [
                actionFor(brief, "Изменить город", "pick_field", "city"),
                actionFor(brief, "Изменить категорию", "pick_field", "category")
            ];
    ...
    if (options.action?.type === "reject" && result) {
        const value = options.action.value;
        const followup = value.reason === "price" ? "Оставил прежний бюджет. Можно указать новую сумму кнопкой ниже." : value.reason === "style" ? "Какой стиль вам ближе? Можно описать пожелание или выбрать условие вручную." : value.reason === "experience" ? "Какой опыт для вас важен? Можно описать пожелание или выбрать условие вручную." : "Исключил этот вариант. Уточните условия, если хотите изменить следующую подборку.";
        message = `${followup}\n\n${result?.summary ?? message}`;
        actions = [
            ...refinementActions(brief),
            actionFor(brief, "Уточнить пожелания", "pick_field", "preferences")
        ];
    ...
    return {
        ...
        dataset_version: catalog.datasetVersion,
