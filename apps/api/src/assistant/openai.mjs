import { z } from "zod";
import { AssistantError, briefSchema } from "./validation.mjs";
...
const briefProperties = {
    ...
    event_format: nullableString,
    date: {
        type: [
            "string",
            "null"
        ],
        description: "ISO YYYY-MM-DD, never a guessed date"
    },
    budget_kzt: {
        type: [
            "integer",
...
export function validateInterpretation(value, catalog) {
    ...
    result.brief.preferences = result.brief.preferences.map((preference)=>({
            ...preference,
...
function instructions(catalog) {
    ...
    const choices = {
        cities: [
            ...new Set(catalog.contractors.map((item)=>item.city))
        ],
        categories: [
            ...new Set(catalog.contractors.flatMap((item)=>item.categories))
        ],
        formats: [
            ...new Set(catalog.contractors.flatMap((item)=>item.event_formats))
        ],
        languages: [
            ...new Set(catalog.contractors.flatMap((item)=>item.languages))
        ]
    };
...
export class OpenAILanguageModel {
    ...
    available;
    version;
    constructor(apiKey, model = process.env.OPENAI_MODEL || "gpt-4o-mini-2024-07-18", fetchImpl = fetch){
        this.apiKey = apiKey;
        this.model = model;
    ...
    async request(path, body) {
        ...
        if (!key) throw new AssistantError("failed-precondition", "AI пока не настроен на сервере. Условия сохранены; можно продолжить кнопками в ручном режиме.", {
    ...
    async extract(input, catalog) {
        ...
        const text = response.output.filter((item)=>item?.type === "message").flatMap((item)=>Array.isArray(item.content) ? item.content : []).filter((item)=>item?.type === "output_text" && typeof item.text === "string").map((item)=>item.text).join("");
        let value;
        try {
            value = JSON.parse(text ?? "");
        } catch  {
            throw new AssistantError("unavailable", "AI не вернул проверяемые условия. Повторите сообщение.");
        }
...
export const createLanguageModel = (apiKey)=>new OpenAILanguageModel(apiKey);
export function mergeInterpretation(previous, interpretation) {
    ...
    brief.skipped_fields = [
        ...new Set(brief.skipped_fields)
    ].filter((field)=>field === "budget_kzt" ? brief.budget_kzt === null || brief.budget_scope === "event" : brief.date === null);
    return brief;
}
