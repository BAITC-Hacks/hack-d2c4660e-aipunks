...
export function createWorkspace(db) {
  ...
  async function prepareAccount(email,password,name) {
    const uid=randomUUID(),salt=randomBytes(16).toString('hex');
    const digest=(await scrypt(password,salt,64)).toString('hex');
    return {uid,email,name,salt,digest};
  }
  function insertAccount({uid,email,name,salt,digest}) {
    db.prepare('INSERT INTO users VALUES(?,?,?,?)').run(uid,email,salt,digest);
    put('account',uid,uid,{uid,name,email,status:'active',deletionRequested:false,revision:1});
  }
