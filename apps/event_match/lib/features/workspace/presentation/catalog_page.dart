...
import '../../matching/presentation/widgets/equal_card_grid.dart';
...
class _CatalogPageState extends State<CatalogPage> {
  ...
  final _catalogAnchor = GlobalKey();
  ...
  Future<void> _filter() async {
    final request = await showSidePanel<MatchRequest>(
      context,
      barrierLabel: 'Закрыть условия события',
  ...
  Future<void> _favorite(Contractor c) async {
    if (_favoritePending.contains(c.id)) return;
    if (widget.uid == null) {
    ...
    setState(() => _favoritePending.add(c.id));
    try {
      ...
      if (mounted && widget.uid == uid) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Не удалось обновить избранное. Повторите действие.'),
          ),
        );
      }
    } finally {
      if (mounted) setState(() => _favoritePending.remove(c.id));
    }
  }
    ...
    children: [
      for (var i = 0; i < items.length; i++)
        ContractorCard(
          ...
          favoriteBusy: _favoritePending.contains(items[i].id),
        ),
    ],
  );
  @override
